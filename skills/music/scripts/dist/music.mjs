...
var PLATFORM_CONFIG = {
	win32: {
		...
		isMatch: (output) => /mpv/i.test(output)
	},
	default: {
		ipcPath: "/tmp/mpv-ipc",
		checkProcess: {
...
var config = PLATFORM_CONFIG[process.platform] || PLATFORM_CONFIG.default;
var IPC_PATH = config.ipcPath;
var COMMANDS = {
	pause: [
		"set_property",
		"pause",
...
function killMpv() {
	import_cross_spawn.default.sync(config.killProcess.cmd, config.killProcess.args);
}
async function sendIpc(command) {
