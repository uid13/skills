...
// src/lib/platform/windows.ts
var WindowsStrategy = class {
  name = "windows";
  exeSuffix = ".exe";
  installHint = "winget install yt-dlp mpv";
  async checkProcess(name) {
    const result = await exec("tasklist", ["/FI", `IMAGENAME eq ${name}.exe`], { timeout: 3e3, noShell: true });
    return new RegExp(`${name}\\.exe`, "i").test(result.stdout);
  }
  async killProcess(name) {
    await exec("taskkill", ["/F", "/IM", `${name}.exe`], { timeout: 5e3, noShell: true });
  }
  getIpcPath(name) {
    return `\\\\.\\pipe\\${name}`;
  }
...
// src/lib/platform/linux.ts
var LinuxStrategy = class {
  name = "linux";
  exeSuffix = "";
  installHint = "sudo apt install yt-dlp mpv";
  async checkProcess(name) {
    const result = await exec("pgrep", ["-x", name], { timeout: 3e3, noShell: true });
    return result.status === 0;
  }
  async killProcess(name) {
    await exec("pkill", ["-x", name], { timeout: 5e3, noShell: true });
  }
  getIpcPath(name) {
    return `/tmp/${name}`;
  }
...
async function exec(command, args, options = {}) {
  ...
  return new Promise((resolve2) => {
    ...
    const child = (0, import_node_child_process.spawn)(actualCommand, actualArgs, spawnOptions);
    let stdout = "";
    let stderr = "";
    let killed = false;
    let timer = null;
    child.stdout?.on("data", (chunk) => {
      ...
      if (stdout.length + stderr.length > maxBuffer) {
        child.kill("SIGKILL");
        killed = true;
        resolve2({ status: null, stdout, stderr, error: new Error(`Output exceeded maxBuffer (${maxBuffer} bytes)`) });
      }
    });
    ...
    if (timeout && timeout > 0) {
      timer = setTimeout(() => {
        child.kill("SIGKILL");
        killed = true;
        resolve2({ status: null, stdout, stderr, error: new Error(`Command timed out after ${timeout}ms`) });
      }, timeout);
    }
    child.on("close", (code) => {
      if (timer) clearTimeout(timer);
      resolve2({ status: code, stdout, stderr });
    });
    child.on("error", (err) => {
      if (timer) clearTimeout(timer);
      resolve2({ status: null, stdout, stderr, error: err });
    });
  });
}
async function commandVersionWorks(command) {
  const result = await exec(command, ["--version"], { timeout: 8e3, noShell: true });
...
async function locatorCandidates(command) {
  ...
  const result = await exec(locator, [...extraArgs, command], { timeout: 5e3, noShell: true });
...
async function resolveExecutable(command) {
  if (resolveCache.has(command)) {
    ...
    const result = await exec(cached, ["--version"], { timeout: 3e3, noShell: true });
...
var IPC_PATH = getPlatform().getIpcPath(PIPE_NAME);
var LABELS = {
  "pause": "\u5DF2\u6682\u505C",
  "resume": "\u5DF2\u7EE7\u7EED\u64AD\u653E",
...
function startMpv(args = []) {
  ...
  mpvProcess = (0, import_node_child_process2.spawn)("mpv", mpvArgs, {
    stdio: ["ignore", "ignore", "ignore"],
    detached: true,
    // 跨平台都使用 detached，让 mpv 在父进程退出后继续运行
    windowsHide: true
  });
...
async function sendIpc(request, timeout = 5e3) {
  return new Promise((resolve2) => {
    ...
    const timer = setTimeout(() => {
      socket.destroy();
      resolve2({ ok: false, error: "IPC \u901A\u4FE1\u8D85\u65F6" });
    }, timeout);
    socket.on("connect", () => {
...
function outputSongInfo(info) {
  ...
  if (jsonMode) {
    ...
    console.log(`  ${colorize("\u{1F3B5}", "cyan")} ${colorize(title, "bold")}`);
    console.log(`  ${colorize("\u{1F464}", "cyan")} ${colorize(artist, "white")}`);
  }
}
...
async function getAudioStreamUrl(url) {
  ...
  try {
    const output = await runYtdlp(args, 6e4);
...
function normalizeOutfile(filePath) {
  if (filePath.startsWith("/tmp/") || filePath.startsWith("/tmp\\")) {
    return (0, import_node_path.resolve)((0, import_node_os.tmpdir)(), filePath.slice(5));
  }
  if (filePath === "/tmp") {
  ...
  try {
    ...
    if (!verified) {
      ...
      if (options.outfile) {
        ...
        (0, import_node_fs.writeFileSync)(normalizedOutfile, JSON.stringify({ status: "failed", ...songInfo, timestamp: (/* @__PURE__ */ new Date()).toISOString() }, null, 2));
      }
      process.exit(1);
    }
    ...
    if (options.outfile) {
      const normalizedOutfile = normalizeOutfile(options.outfile);
      (0, import_node_fs.writeFileSync)(normalizedOutfile, JSON.stringify({ status: "success", ...songInfo, timestamp: (/* @__PURE__ */ new Date()).toISOString() }, null, 2));
    }
...
program2.name("music").description("\u64AD\u653E\u3001\u6682\u505C\u3001\u63A7\u5236\u5728\u7EBF\u97F3\u4E50").version("1.0.0");
program2.command("play [query..]", { isDefault: true }).description("\u64AD\u653E\u6B4C\u66F2\uFF08\u9ED8\u8BA4\u547D\u4EE4\uFF09").option("-j, --json", "JSON \u8F93\u51FA\u6A21\u5F0F\uFF08\u4F9B Agent \u89E3\u6790\uFF09", false).option("--timeout <ms>", "\u8D85\u65F6\u65F6\u95F4\uFF08\u6BEB\u79D2\uFF09", parseInt, DEFAULT_TIMEOUT).option("--outfile <path>", "\u5C06\u6B4C\u66F2\u4FE1\u606F\u5199\u5165\u6587\u4EF6").action((query, options) => {
...
controlActions.forEach((action) => {
  program2.command(action).description(`\u6267\u884C ${action} \u63A7\u5236\u547D\u4EE4`).option("-j, --json", "JSON \u8F93\u51FA\u6A21\u5F0F", false).action((options) => {
    controlCommand(action, options);
  });
});
program2.command("status").description("\u67E5\u8BE2\u64AD\u653E\u72B6\u6001").option("-j, --json", "JSON \u8F93\u51FA\u6A21\u5F0F", false).action((options) => {
  statusCommand(options);
});
program2.parse();
//# sourceMappingURL=music.cjs.map
