...
│   ├── SKILL.md           # 技能入口文档
│   ├── wc.html            # 浏览器查看页面（Vue 3 via CDN）
│   └── wc-data.js         # 词汇数据文件（运行时由 Agent 更新）
├── src/                   # Vite 构建入口
...
- 不要手动修改 `skills/wc/` 下的文件，它们都是构建产物
- `src/dummy.js` 不可删除，Vite 构建需要至少一个入口文件
- `wc-data.js` 初始为占位数据，运行时由 Agent 根据用户输入生成并更新
- `wc.html` 通过 CDN 加载 Vue 3，需要网络连接才能正常显示
